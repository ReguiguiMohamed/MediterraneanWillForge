...
def test_required_columns_missing_detected(bronze_df):
    results = check_required_columns(bronze_df, "bronze", "t", ["station_id", "nonexistent_col"])
...
def test_null_rate_warns_above_warn_threshold():
    df = pd.DataFrame({
        "pm2_5": [None, None, 10.0, 5.0, 3.0]  # 40% null → warn
    })
    r = check_null_rate(df, "bronze", "t", "pm2_5", warn_threshold=0.30, fail_threshold=0.70)
    assert not r.passed
