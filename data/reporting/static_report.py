...
def plot_anomaly_detection(
    anomalies: pd.DataFrame, ready_dates: set[str], output_path: Path
) -> None:
    if anomalies.empty:
        return

    public_anomalies = anomalies[anomalies["partition_date"].isin(ready_dates)].copy()
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        [str(d)[-5:] for d in rates["partition_date"]],
        rotation=70,
        ha="right",
        fontsize=7,
    )
    ...
    fig.suptitle(
        "Anomaly Detection - Stable Reporting Dates",
        fontsize=13,
        fontweight="bold",
        y=1.02,
    )
...
def main() -> None:
    ...
    plot_anomaly_detection(anomalies, ready_dates, REPORT_DIR / "anomaly_detection.png")
